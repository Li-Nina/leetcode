...
if __name__ == '__main__':
    ...
    bar = Bar('各地出差人天数排名柱状图', title_pos="70%")
    bar.add('出差人', province_name[0:10], province_value[0:10], xaxis_rotate=40, yaxis_name='出差数(天)',
            yaxis_name_pos='middle',
            yaxis_name_gap=40, is_label_show=True, legend_pos="85%", legend_orient="vertical", mark_point=['max'])
    grid.add(bar, grid_left="70%")
    split_number = 5  # 分段的个数
    max_value = cal_proper_max_value(max(province_value), split_number)
    map = Map('china', width=800, height=600, title_pos="10%")
    map.add(
        "",
        province_name,
        province_value,
