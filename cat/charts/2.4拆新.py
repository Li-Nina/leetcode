from pyecharts import Bar, Pie, Grid
import pandas as pd
from pyecharts_javascripthon.dom import window
home_icon = "image://data:image/png;base64,"

def switch_button():
    window.open('2.4_3.2各组出差费用组合图.html', '_self')

def format(w):
    if w.name:
        return w.name + ':' + w.value
    else:
        return 0

group01 = []
sheet0 = pd.read_excel("./data_resources/2.4各组出差费用柱状图和饼图新.xls")
group02 = sheet0["组名"].values.tolist()
for i in ["售前组", "硬件组", "领导组", "销售组",
      "大数据云计算组", "软件组", "市场商务", "综合管理"]:
    group01.append((i, sheet0[i].values.tolist()))
bar0 = Bar('各组出差人费用排名', title_pos="10%")
for i in group01:
    if i[1] is not None:
        bar0.add(i[0], group02, i[1], xaxis_rotate=30, yaxis_name='出差数(天)', yaxis_name_pos='middle',
                 yaxis_name_gap=40, legend_pos="40%", legend_top="2%", is_label_show=True, label_pos='inside',
                 is_stack=True)
bar0.render("./all_charts/" + "2.4各组出差费用情况图" + ".html")
# sheet1 = pd.read_excel("./data_resources/2.4各组出差费用柱状图和饼图.xls")
# group1 = sheet1["组名"].values.tolist()
# days1 = sheet1["出差费用（四舍五入取整）"].values.tolist()
# pie1 = Pie('各组出差费用比例', title_pos="55%")
# pie1.add('组名', group1, days1, radius=[45, 65], center=[65, 50], is_label_show=True, legend_pos="85%",
#          legend_orient="vertical", legend_top="20%", mark_point=['max'])
#
# bar0._option.get("toolbox").get("feature").update(
#     {
#         "myTool": {"show": True, "title": "switch", "icon": home_icon, "onclick": switch_button},
#     }
# )
#
# grid = Grid(width=1200)
# grid.add(bar0, grid_right="60%")
# grid.add(pie1, grid_left="70%")
# grid.render("./all_charts/" + "2.4_3.2各组出差费用组合图" + ".html")
# #
# sheet2 = pd.read_excel("./data_resources/2.4各组出差费用柱状图和饼图.xls")
# group2 = sheet2["组名"].values.tolist()
# days2 = sheet2["出差费用（四舍五入取整）"].values.tolist()
# bar2 = Bar('各组出差费用排名', title_pos="10%")
# bar2.add('组名', group2, days2, xaxis_rotate=30, yaxis_name='出差费用(万元)', yaxis_name_pos='middle', yaxis_name_gap=40,
#          legend_pos="50%", is_label_show=True, label_pos='inside')
# bar2.render("./all_charts/" + "2.4各组出差费用情况图" + ".html")
