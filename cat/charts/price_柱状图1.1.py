...
sheet0 = pd.read_excel("./data_resources/出差省成本.xls")
people0 = sheet0["出差省"][0:10].values.tolist()
days0 = sheet0["出差成本（万元）"][0:10].values.tolist()
bar0 = Bar('出差省成本',title_pos="10%")
bar0.add('出差省',people0,days0,xaxis_rotate = 40, yaxis_name = '出差成本（万元）',yaxis_name_pos = 'middle',yaxis_name_gap = 40,legend_pos="35%",label_text_size = 10, is_label_show = True,label_pos= 'inside')
...
grid.render("./all_charts/"+"1.1出差地区与出差成本"+".html")