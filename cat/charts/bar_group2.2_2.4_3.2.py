from pyecharts import Bar, Pie, Grid
import pandas as pd
from pyecharts_javascripthon.dom import window
home_icon = "image://data:image/png;base64,"

def switch_button():
    window.open('2.4_3.2各组出差费用组合图.html', '_self')

def format(w):
    if w.name:
        return w.name + ':' + w.value
    else:
        return 0
#
# sheet2 = pd.read_excel("./data_resources/2.4各组出差费用柱状图和饼图.xls")
# group2 = sheet2["组名"].values.tolist()
# days2 = sheet2["出差费用（四舍五入取整）"].values.tolist()
# bar2 = Bar('各组出差费用排名', title_pos="10%")
# bar2.add('组名', group2, days2, xaxis_rotate=30, yaxis_name='出差费用(万元)', yaxis_name_pos='middle', yaxis_name_gap=40,
#          legend_pos="50%", is_label_show=True, label_pos='inside')
# bar2.render("./all_charts/" + "2.4各组出差费用情况图" + ".html")


sheet0 = pd.read_excel("./data_resources/2.2每个组的出差天数柱状图和饼图.xls")
group0 = sheet0["组名"].values.tolist()
days0 = sheet0["出差天数（取整）"].values.tolist()
bar0 = Bar('各组出差人费用排名', title_pos="20%")
bar0.add('组名',group0,days0,xaxis_rotate =30, tooltip_formatter=format,yaxis_name = '出差数(天)',yaxis_name_pos = 'middle',yaxis_name_gap = 40,legend_pos="35%",is_label_show = True,label_pos= 'inside',mark_point = ['max'])

sheet2 = pd.read_excel("./data_resources/2.4各组出差费用柱状图和饼图.xls")
group2 = sheet2["组名"].values.tolist()
days2 = sheet2["出差费用（四舍五入取整）"].values.tolist()
bar2 = Bar('各组出差费用排名', title_pos="65%")
bar2.add('组名', group2, days2, xaxis_rotate =30, yaxis_name = '出差费用(万元)',yaxis_name_pos = 'middle',yaxis_name_gap = 40, legend_pos="80%",is_label_show = True,label_pos= 'inside',mark_point = ['max'])

sheet1 = pd.read_excel("./data_resources/2.2每个组的出差天数柱状图和饼图.xls")
group1 = sheet1["组名"].values.tolist()
days1 = sheet1["出差天数（取整）"].values.tolist()
pie1 = Pie('各组出差人天数比例',title_top="50%",title_pos="65%")
pie1.add('组名',group1,days1,radius=[20, 35],center=[67, 75],is_label_show = True,legend_top="80%",legend_pos="80%",legend_orient="vertical",mark_point = ['max'])

bar0._option.get("toolbox").get("feature").update(
    {
        "myTool": {"show": True, "title": "switch", "icon": home_icon, "onclick": switch_button},
    }
)


sheet3 = pd.read_excel("./data_resources/2.4各组出差费用柱状图和饼图.xls")
group3 = sheet3["组名"].values.tolist()
days3= sheet3["出差费用（四舍五入取整）"].values.tolist()
pie3 = Pie('各组出差费用比例',title_top="50%",title_pos="20%")
pie3.add('组名',group3,days3,radius=[20, 35],center=[22, 75],is_label_show = True, legend_top="80%", legend_pos="35%",legend_orient="vertical",mark_point = ['max'])

# bar2._option.get("toolbox").get("feature").update(
#     {
#         "myTool": {"show": True, "title": "switch", "icon": home_icon, "onclick": switch_button},
#     }
# )

# #grid = Grid(width=1200)
grid = Grid(height=720, width=1500)
grid.add(bar0, grid_bottom="60%", grid_left="55%")
grid.add(bar2, grid_bottom="60%", grid_right="55%")
grid.add(pie1, grid_top="60%", grid_left="60%")
grid.add(pie3, grid_top="60%", grid_right="60%")
grid.render("./all_charts/"+"2.2_2.4_3.2各组出差情况组合图"+".html")
