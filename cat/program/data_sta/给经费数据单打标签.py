...
xls_file = xlrd.open_workbook("合并60多个项目的.xlsx")
...
teach_list=["培训","培训费"]
out_list=["差旅"]
office_other_list=["办公","办公费","交通","交通费"]
...
for rowx in  range(1,nrows):
    ...
    for seg in psg.lcut(receipt_type):
        if seg.word in receipt_list:
            ...
            break
    for seg in psg.lcut(receipt_type):
        if seg.word in eat_list:
            print(seg.word)
            new_type = "餐费"
            break
    for seg in psg.lcut(receipt_type):
        if seg.word in office_other_list:
            print(seg.word)
            new_type = "办公费"
            break
    for seg in psg.lcut(receipt_type):
