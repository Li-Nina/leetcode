...
import xlrd
import xlwt
xls_file = xlrd.open_workbook("source_data_out_besidebeijing.xls")
xls_sheet = xls_file.sheets()[0] #打开第几个bu
col_name= xls_sheet.col_values(19) #第7列为根据：出差人
col_day= xls_sheet.col_values(18) #第11列为统计变量：出差天数
del col_name[0]
print(col_name)
del col_day[0]
print(col_day)
...
for k,v in day_dict.items():
    ...
    new_row+=1
workbook.save('./result/每组出差费用不计算北京.xls')