...
xls_file = xlrd.open_workbook("merge_project_all.xls")
...
worksheet.write(new_row,col_num,"单据类型说明")#增加一列
new_row+=1#新行
name_dict={}
#从1行开始按照行遍历
for rowx in  range(1,nrows):
    ...
    if (old_type == u'差旅'):
        for i in range(col_num):
            worksheet.write(new_row,i,data[i].value) #之前的值单元格所在的row,column,和value.
        worksheet.write(new_row,col_num,old_type)#把差旅单独一列写出来
        new_row +=1
        continue
    ...
    for i in range(col_num):
        worksheet.write(new_row, i, data[i].value)  # 单元格所在的row,column,和value.
    worksheet.write(new_row, col_num, new_type)#把新类型写出来
    new_row += 1
